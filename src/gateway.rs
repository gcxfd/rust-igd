...
use crate::{
  common::{self, messages, parsing, parsing::RequestResult},
  errors::{
...
impl Gateway {
  ...
  fn add_random_port_mapping(
    ...
    if let Err(err) = self.add_port_mapping(
      protocol,
      external_port,
      local_addr,
      lease_duration,
      ...
      match parsing::convert_add_random_port_mapping_error(err) {
        ...
        None => {
          return self.add_same_port_mapping(protocol, local_addr, lease_duration, description)
        }
      }
    }
